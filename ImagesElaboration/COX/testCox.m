...
[immagineFinale1, watermark1] = inserisciWatermarkCox(immagineBase, 1000, 1);
[immagineFinale2, watermark2] = inserisciWatermarkCox(immagineBase, 1000, 1);

[width1, heigth1, yu] = size(immagineFinale1);

[confidenzaCorretta] = confrontoWatermarkCox(immagineBase, immagineFinale1, watermark1, 1);
[confidenzaErrata] = confrontoWatermarkCox(immagineBase, immagineFinale2, watermark1, 1);
...
for i=1:length(density)
    [confidenzaSP(i)] = confrontoWatermarkCox(immagineBase, imnoise(immagineFinale1 ./ 255, 'salt & pepper', density(i)) .* 255, watermark1, 1);
end

confidenzaGauss = zeros(1,length(density));
for i=1:length(density)
    [confidenzaGauss(i)] = confrontoWatermarkCox(immagineBase, imnoise(immagineFinale1 ./ 255, 'gaussian', density(i)) .* 255, watermark1, 1);
end

confidenzaSpeck = zeros(1,length(density));
for i=1:length(density)
    [confidenzaSpeck(i)] = confrontoWatermarkCox(immagineBase, imnoise(immagineFinale1 ./ 255, 'speckle', density(i)) .* 255, watermark1, 1);
end
...
for j=1:length(percDeleteList)
    ...
    [confidenzaJPEG(j)] = confrontoWatermarkCox(immagineBase, immagineFinaleCompressa, watermark1, 1);
end
...
for i = [1:length(xtr)]
    for j = [1:length(ytr)]
        ...
        [confidenzaTrasl(i,j)] = confrontoWatermarkCox(immagineBase, imTrasl, watermark1, 1);
    end
end
...
for i=[1:length(theta)]
    ...
    [confidenzaRot(i)] = confrontoWatermarkCox(immagineBase, imRot, watermark1, 1);
end
...
for i=[1:length(scale)]
    ...
    [confidenzaScale(i)] = confrontoWatermarkCox(immagineBase, imScal, watermark1, 1);
end
...
for i=[1:length(xCut)]
    for j=[1:length(yCut)]
        ...
        [confidenzaCut(i,j)] = confrontoWatermarkCox(immagineBase, imCut, watermark1, 1);
    end
end